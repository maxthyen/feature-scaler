 ...
 */
function decode(encoded, decoders) {
  return _.map(encoded, row => decodeRow(row, decoders));
}
...
function decodeRow(row, decoders) {
  ...
  function getValue(row, ix, decoder) {
    switch (decoder.type) {
      ...
      case 'boolean': return !!row[ix];
      case 'number': return decoder.std * row[ix] + decoder.mean;
      default: return row[ix];
    }
  }
}
...
function standardizeField(key, data, decoders) {
  ...
  switch (type) {
    case 'string': {
      const oneHotEncoder = buildOneHot(values);
